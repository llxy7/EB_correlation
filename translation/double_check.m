...
dumpdir="/tomerv3/Sida/b-mode-log-griding/f_"<>ToString[log10f]<>"_L_"<>$CommandLine[[-6]]<>"_tau_"<>$CommandLine[[-5]]<>"_N_"<>$CommandLine[[-4]]<>"_kmax_"<>$CommandLine[[-3]]<>"_nat_tau/test_jl/";
...
jl=Interpolation[Import[dumpdir<>"jl_"<>$CommandLine[[-2]]<>"_"<>$CommandLine[[-1]]<>"_d.dat"]];
jl$num[t_?NumberQ]:=jl[t];

func[tp_]:=NIntegrate[1/func$a$num[t]*(g2$num[tp]*g1p$num[t]-g1$num[tp]*g2p$num[t]-func$ap$num[t]/func$a$num[t]*(g2$num[tp]*g1$num[t]-g1$num[tp]*g2$num[t]))*jl$num[t]/((τ0-t)^2)/(klist[[kk]]^2),{t,tp,τ0}];
Print[func[5*10^40]//Quiet//AbsoluteTiming];

func2[tp_]:=NIntegrate[1/func$a$num[t]*(g2$num[tp]*g1p$num[t]-g1$num[tp]*g2p$num[t]-func$ap$num[t]/func$a$num[t]*(g2$num[tp]*g1$num[t]-g1$num[tp]*g2$num[t]))*jl$num[t]/((τ0-t)^2)/(klist[[kk]]^2),{t,tp,τ0},WorkingPrecision->30];
Print[func2[5*10^40]//Quiet//AbsoluteTiming];

func3[tp_]:=NIntegrate[1/func$a$num[t]*(g2$num[tp]*g1p$num[t]-g1$num[tp]*g2p$num[t]-func$ap$num[t]/func$a$num[t]*(g2$num[tp]*g1$num[t]-g1$num[tp]*g2$num[t]))*SphericalBesselJ[ll,(τ0-t)*klist[[kk]]]/((τ0-t)^2)/(klist[[kk]]^2),{t,tp,τ0}];
Print[func3[5*10^40]//Quiet//AbsoluteTiming];

func4[tp_]:=NIntegrate[1/func$a$num[t]*(g2$num[tp]*g1p$num[t]-g1$num[tp]*g2p$num[t]-func$ap$num[t]/func$a$num[t]*(g2$num[tp]*g1$num[t]-g1$num[tp]*g2$num[t]))*SphericalBesselJ[ll,(τ0-t)*klist[[kk]]]/((τ0-t)^2)/(klist[[kk]]^2),{t,tp,τ0},WorkingPrecision->30];
Print[func4[5*10^40]//Quiet//AbsoluteTiming];
